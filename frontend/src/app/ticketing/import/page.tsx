'use client';
/** 機票進票作業 | L_inticket.asp | DB: ticket_imports */
import React from 'react';
import { Typography, Table, Button, Space, Tag, message } from 'antd';
import { PlusOutlined, EditOutlined, DeleteOutlined } from '@ant-design/icons';
const { Title } = Typography;

// === [API] GET /api/ticketing/import === DB: ticket_imports === TODO: [替換] 改為實際 API ===
const mockData = Array.from({length:10},(_,i)=>({ key:String(i+1), id:'INT' + String(i+1).padStart(4,'0'), name:'機票進票作業項目' + (i+1), status:i%3===0?'停用':'啟用', date:'2026-03-' + String(20+i).padStart(2,'0') }));
const columns = [
...
export default function Page() {
  return (<div>
    <div className="table-toolbar"><Title level={4} style={{margin:0}}>機票進票作業</Title>
    <Button type="primary" icon={<PlusOutlined />} onClick={()=>message.info('新增功能 (待接後端)')}>新增</Button></div>
    {/* === [API] POST /api/ticketing/import === DB: INSERT INTO ticket_imports === TODO: [替換] === */}
    {/* === [API] PUT /api/ticketing/import/:id === DB: UPDATE ticket_imports SET ... === TODO: [替換] === */}
    {/* === [API] DELETE /api/ticketing/import/:id === DB: DELETE FROM ticket_imports WHERE INTKT_NO=:id === TODO: [替換] === */}

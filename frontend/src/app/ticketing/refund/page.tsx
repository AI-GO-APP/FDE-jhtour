'use client';
/** 退票作業維護 | L_rord.asp | DB: ticket_refunds */
import React from 'react';
import { Typography, Table, Button, Space, Tag, message } from 'antd';
import { PlusOutlined, EditOutlined, DeleteOutlined } from '@ant-design/icons';
const { Title } = Typography;

// === [API] GET /api/ticketing/refund === DB: ticket_refunds === TODO: [替換] 改為實際 API ===
const mockData = Array.from({length:10},(_,i)=>({ key:String(i+1), id:'ROR' + String(i+1).padStart(4,'0'), name:'退票作業維護項目' + (i+1), status:i%3===0?'停用':'啟用', date:'2026-03-' + String(20+i).padStart(2,'0') }));
const columns = [
...
export default function Page() {
  return (<div>
    <div className="table-toolbar"><Title level={4} style={{margin:0}}>退票作業維護</Title>
    <Button type="primary" icon={<PlusOutlined />} onClick={()=>message.info('新增功能 (待接後端)')}>新增</Button></div>
    {/* === [API] POST /api/ticketing/refund === DB: INSERT INTO ticket_refunds === TODO: [替換] === */}
    {/* === [API] PUT /api/ticketing/refund/:id === DB: UPDATE ticket_refunds SET ... === TODO: [替換] === */}
    {/* === [API] DELETE /api/ticketing/refund/:id === DB: DELETE FROM ticket_refunds WHERE RORD_NO=:id === TODO: [替換] === */}

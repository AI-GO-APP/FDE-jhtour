'use client';
/** 票價修改通知 | L_tsubd_updmsg.asp | DB: price_notifications */
import React from 'react';
import { Typography, Table, Button, Space, Tag, message } from 'antd';
import { PlusOutlined, EditOutlined, DeleteOutlined } from '@ant-design/icons';
const { Title } = Typography;

// === [API] GET /api/ticketing/price-notify === DB: price_notifications === TODO: [替換] 改為實際 API ===
const mockData = Array.from({length:10},(_,i)=>({ key:String(i+1), id:'NOT' + String(i+1).padStart(4,'0'), name:'票價修改通知項目' + (i+1), status:i%3===0?'停用':'啟用', date:'2026-03-' + String(20+i).padStart(2,'0') }));
const columns = [
...
export default function Page() {
  return (<div>
    <div className="table-toolbar"><Title level={4} style={{margin:0}}>票價修改通知</Title>
    <Button type="primary" icon={<PlusOutlined />} onClick={()=>message.info('新增功能 (待接後端)')}>新增</Button></div>
    {/* === [API] POST /api/ticketing/price-notify === DB: INSERT INTO price_notifications === TODO: [替換] === */}
    {/* === [API] PUT /api/ticketing/price-notify/:id === DB: UPDATE price_notifications SET ... === TODO: [替換] === */}
    {/* === [API] DELETE /api/ticketing/price-notify/:id === DB: DELETE FROM price_notifications WHERE NOTIFY_ID=:id === TODO: [替換] === */}

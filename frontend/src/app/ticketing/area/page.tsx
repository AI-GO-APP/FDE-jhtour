'use client';
/** 機票行程地區 | L_tarea.asp | DB: ticket_areas */
import React from 'react';
import { Typography, Table, Button, Space, Tag, message } from 'antd';
import { PlusOutlined, EditOutlined, DeleteOutlined } from '@ant-design/icons';
const { Title } = Typography;

// === [API] GET /api/ticketing/area === DB: ticket_areas === TODO: [替換] 改為實際 API ===
const mockData = Array.from({length:10},(_,i)=>({ key:String(i+1), id:'TAR' + String(i+1).padStart(4,'0'), name:'機票行程地區項目' + (i+1), status:i%3===0?'停用':'啟用', date:'2026-03-' + String(20+i).padStart(2,'0') }));
const columns = [
...
export default function Page() {
  return (<div>
    <div className="table-toolbar"><Title level={4} style={{margin:0}}>機票行程地區</Title>
    <Button type="primary" icon={<PlusOutlined />} onClick={()=>message.info('新增功能 (待接後端)')}>新增</Button></div>
    {/* === [API] POST /api/ticketing/area === DB: INSERT INTO ticket_areas === TODO: [替換] === */}
    {/* === [API] PUT /api/ticketing/area/:id === DB: UPDATE ticket_areas SET ... === TODO: [替換] === */}
    {/* === [API] DELETE /api/ticketing/area/:id === DB: DELETE FROM ticket_areas WHERE TAREA_CD=:id === TODO: [替換] === */}

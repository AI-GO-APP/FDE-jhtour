'use client';
/** 旅客辦證記錄 | L_ovisa.asp | DB: visa_records */
import React from 'react';
import { Typography, Table, Button, Space, Tag, message } from 'antd';
import { PlusOutlined, EditOutlined, DeleteOutlined } from '@ant-design/icons';
const { Title } = Typography;

// === [API] GET /api/visa/record === DB: visa_records === TODO: [替換] 改為實際 API ===
const mockData = Array.from({length:10},(_,i)=>({ key:String(i+1), id:'OVI' + String(i+1).padStart(4,'0'), name:'旅客辦證記錄項目' + (i+1), status:i%3===0?'停用':'啟用', date:'2026-03-' + String(20+i).padStart(2,'0') }));
const columns = [
...
export default function Page() {
  return (<div>
    <div className="table-toolbar"><Title level={4} style={{margin:0}}>旅客辦證記錄</Title>
    <Button type="primary" icon={<PlusOutlined />} onClick={()=>message.info('新增功能 (待接後端)')}>新增</Button></div>
    {/* === [API] POST /api/visa/record === DB: INSERT INTO visa_records === TODO: [替換] === */}
    {/* === [API] PUT /api/visa/record/:id === DB: UPDATE visa_records SET ... === TODO: [替換] === */}
    {/* === [API] DELETE /api/visa/record/:id === DB: DELETE FROM visa_records WHERE OVISA_NO=:id === TODO: [替換] === */}

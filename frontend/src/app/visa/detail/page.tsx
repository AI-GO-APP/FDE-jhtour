'use client';
/** 交辦處理明細 | L_ovisad.asp | DB: visa_details */
import React from 'react';
import { Typography, Table, Button, Space, Tag, message } from 'antd';
import { PlusOutlined, EditOutlined, DeleteOutlined } from '@ant-design/icons';
const { Title } = Typography;

// === [API] GET /api/visa/detail === DB: visa_details === TODO: [替換] 改為實際 API ===
const mockData = Array.from({length:10},(_,i)=>({ key:String(i+1), id:'OVI' + String(i+1).padStart(4,'0'), name:'交辦處理明細項目' + (i+1), status:i%3===0?'停用':'啟用', date:'2026-03-' + String(20+i).padStart(2,'0') }));
const columns = [
...
export default function Page() {
  return (<div>
    <div className="table-toolbar"><Title level={4} style={{margin:0}}>交辦處理明細</Title>
    <Button type="primary" icon={<PlusOutlined />} onClick={()=>message.info('新增功能 (待接後端)')}>新增</Button></div>
    {/* === [API] POST /api/visa/detail === DB: INSERT INTO visa_details === TODO: [替換] === */}
    {/* === [API] PUT /api/visa/detail/:id === DB: UPDATE visa_details SET ... === TODO: [替換] === */}
    {/* === [API] DELETE /api/visa/detail/:id === DB: DELETE FROM visa_details WHERE OVISA_NO=:id === TODO: [替換] === */}

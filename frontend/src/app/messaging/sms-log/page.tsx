'use client';
/** 簡訊處理記錄 | L_sms_proc_log_detail.asp | DB: sms_logs */
import React from 'react';
import { Typography, Table, Button, Space, Tag, message } from 'antd';
import { PlusOutlined, EditOutlined, DeleteOutlined } from '@ant-design/icons';
const { Title } = Typography;

// === [API] GET /api/messaging/sms-log === DB: sms_logs === TODO: [替換] 改為實際 API ===
const mockData = Array.from({length:10},(_,i)=>({ key:String(i+1), id:'LOG' + String(i+1).padStart(4,'0'), name:'簡訊處理記錄項目' + (i+1), status:i%3===0?'停用':'啟用', date:'2026-03-' + String(20+i).padStart(2,'0') }));
const columns = [
...
export default function Page() {
  return (<div>
    <div className="table-toolbar"><Title level={4} style={{margin:0}}>簡訊處理記錄</Title>
    <Button type="primary" icon={<PlusOutlined />} onClick={()=>message.info('新增功能 (待接後端)')}>新增</Button></div>
    {/* === [API] POST /api/messaging/sms-log === DB: INSERT INTO sms_logs === TODO: [替換] === */}
    {/* === [API] PUT /api/messaging/sms-log/:id === DB: UPDATE sms_logs SET ... === TODO: [替換] === */}
    {/* === [API] DELETE /api/messaging/sms-log/:id === DB: DELETE FROM sms_logs WHERE LOG_ID=:id === TODO: [替換] === */}

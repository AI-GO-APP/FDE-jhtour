'use client';
/** 簡訊行銷 | L_sms_promo_list.asp | DB: sms_promos */
import React from 'react';
import { Typography, Table, Button, Space, Tag, message } from 'antd';
import { PlusOutlined, EditOutlined, DeleteOutlined } from '@ant-design/icons';
const { Title } = Typography;

// === [API] GET /api/messaging/sms-promo === DB: sms_promos === TODO: [替換] 改為實際 API ===
const mockData = Array.from({length:10},(_,i)=>({ key:String(i+1), id:'PRO' + String(i+1).padStart(4,'0'), name:'簡訊行銷項目' + (i+1), status:i%3===0?'停用':'啟用', date:'2026-03-' + String(20+i).padStart(2,'0') }));
const columns = [
...
export default function Page() {
  return (<div>
    <div className="table-toolbar"><Title level={4} style={{margin:0}}>簡訊行銷</Title>
    <Button type="primary" icon={<PlusOutlined />} onClick={()=>message.info('新增功能 (待接後端)')}>新增</Button></div>
    {/* === [API] POST /api/messaging/sms-promo === DB: INSERT INTO sms_promos === TODO: [替換] === */}
    {/* === [API] PUT /api/messaging/sms-promo/:id === DB: UPDATE sms_promos SET ... === TODO: [替換] === */}
    {/* === [API] DELETE /api/messaging/sms-promo/:id === DB: DELETE FROM sms_promos WHERE PROMO_ID=:id === TODO: [替換] === */}

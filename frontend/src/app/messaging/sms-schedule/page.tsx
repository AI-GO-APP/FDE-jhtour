'use client';
/** 預約發送管理 | CancelRsvnSmsList.asp | DB: sms_schedules */
import React from 'react';
import { Typography, Table, Button, Space, Tag, message } from 'antd';
import { PlusOutlined, EditOutlined, DeleteOutlined } from '@ant-design/icons';
const { Title } = Typography;

// === [API] GET /api/messaging/sms-schedule === DB: sms_schedules === TODO: [替換] 改為實際 API ===
const mockData = Array.from({length:10},(_,i)=>({ key:String(i+1), id:'RSV' + String(i+1).padStart(4,'0'), name:'預約發送管理項目' + (i+1), status:i%3===0?'停用':'啟用', date:'2026-03-' + String(20+i).padStart(2,'0') }));
const columns = [
...
export default function Page() {
  return (<div>
    <div className="table-toolbar"><Title level={4} style={{margin:0}}>預約發送管理</Title>
    <Button type="primary" icon={<PlusOutlined />} onClick={()=>message.info('新增功能 (待接後端)')}>新增</Button></div>
    {/* === [API] POST /api/messaging/sms-schedule === DB: INSERT INTO sms_schedules === TODO: [替換] === */}
    {/* === [API] PUT /api/messaging/sms-schedule/:id === DB: UPDATE sms_schedules SET ... === TODO: [替換] === */}
    {/* === [API] DELETE /api/messaging/sms-schedule/:id === DB: DELETE FROM sms_schedules WHERE RSVN_ID=:id === TODO: [替換] === */}

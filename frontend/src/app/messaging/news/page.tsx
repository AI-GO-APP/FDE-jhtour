'use client';
/** 公佈欄 | L_news.asp | DB: bbs_messages */
import React from 'react';
import { Typography, Table, Button, Space, Tag, message } from 'antd';
import { PlusOutlined, EditOutlined, DeleteOutlined } from '@ant-design/icons';
const { Title } = Typography;

// === [API] GET /api/messaging/news === DB: bbs_messages === TODO: [替換] 改為實際 API ===
const mockData = Array.from({length:10},(_,i)=>({ key:String(i+1), id:'BBS' + String(i+1).padStart(4,'0'), name:'公佈欄項目' + (i+1), status:i%3===0?'停用':'啟用', date:'2026-03-' + String(20+i).padStart(2,'0') }));
const columns = [
...
export default function Page() {
  return (<div>
    <div className="table-toolbar"><Title level={4} style={{margin:0}}>公佈欄</Title>
    <Button type="primary" icon={<PlusOutlined />} onClick={()=>message.info('新增功能 (待接後端)')}>新增</Button></div>
    {/* === [API] POST /api/messaging/news === DB: INSERT INTO bbs_messages === TODO: [替換] === */}
    {/* === [API] PUT /api/messaging/news/:id === DB: UPDATE bbs_messages SET ... === TODO: [替換] === */}
    {/* === [API] DELETE /api/messaging/news/:id === DB: DELETE FROM bbs_messages WHERE BBS_ID=:id === TODO: [替換] === */}

'use client';
/** 使用者群組管理 | L_USERGRP.asp | DB: user_groups */
import React from 'react';
import { Typography, Table, Button, Space, Tag, message } from 'antd';
import { PlusOutlined, EditOutlined, DeleteOutlined } from '@ant-design/icons';
const { Title } = Typography;

// === [API] GET /api/settings/user-group === DB: user_groups === TODO: [替換] 改為實際 API ===
const mockData = Array.from({length:10},(_,i)=>({ key:String(i+1), id:'GRP' + String(i+1).padStart(4,'0'), name:'使用者群組管理項目' + (i+1), status:i%3===0?'停用':'啟用', date:'2026-03-' + String(20+i).padStart(2,'0') }));
const columns = [
...
export default function Page() {
  return (<div>
    <div className="table-toolbar"><Title level={4} style={{margin:0}}>使用者群組管理</Title>
    <Button type="primary" icon={<PlusOutlined />} onClick={()=>message.info('新增功能 (待接後端)')}>新增</Button></div>
    {/* === [API] POST /api/settings/user-group === DB: INSERT INTO user_groups === TODO: [替換] === */}
    {/* === [API] PUT /api/settings/user-group/:id === DB: UPDATE user_groups SET ... === TODO: [替換] === */}
    {/* === [API] DELETE /api/settings/user-group/:id === DB: DELETE FROM user_groups WHERE GRP_ID=:id === TODO: [替換] === */}

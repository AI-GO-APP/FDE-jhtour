'use client';
/** 寄送郵件列表 | L_MailStatus.asp | DB: mail_records */
import React from 'react';
import { Typography, Table, Button, Space, Tag, message } from 'antd';
import { PlusOutlined, EditOutlined, DeleteOutlined } from '@ant-design/icons';
const { Title } = Typography;

// === [API] GET /api/settings/mail-list === DB: mail_records === TODO: [替換] 改為實際 API ===
const mockData = Array.from({length:10},(_,i)=>({ key:String(i+1), id:'MAI' + String(i+1).padStart(4,'0'), name:'寄送郵件列表項目' + (i+1), status:i%3===0?'停用':'啟用', date:'2026-03-' + String(20+i).padStart(2,'0') }));
const columns = [
...
export default function Page() {
  return (<div>
    <div className="table-toolbar"><Title level={4} style={{margin:0}}>寄送郵件列表</Title>
    <Button type="primary" icon={<PlusOutlined />} onClick={()=>message.info('新增功能 (待接後端)')}>新增</Button></div>
    {/* === [API] POST /api/settings/mail-list === DB: INSERT INTO mail_records === TODO: [替換] === */}
    {/* === [API] PUT /api/settings/mail-list/:id === DB: UPDATE mail_records SET ... === TODO: [替換] === */}
    {/* === [API] DELETE /api/settings/mail-list/:id === DB: DELETE FROM mail_records WHERE MAIL_ID=:id === TODO: [替換] === */}

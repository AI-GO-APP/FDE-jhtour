'use client';
/** 團體分房表 | L_gruproom.asp | DB: group_rooms */
import React from 'react';
import { Typography, Table, Button, Space, Tag, message } from 'antd';
import { PlusOutlined, EditOutlined, DeleteOutlined } from '@ant-design/icons';
const { Title } = Typography;

// === [API] GET /api/group-sales/room === DB: group_rooms === TODO: [替換] 改為實際 API ===
const mockData = Array.from({length:10},(_,i)=>({ key:String(i+1), id:'GRU' + String(i+1).padStart(4,'0'), name:'團體分房表項目' + (i+1), status:i%3===0?'停用':'啟用', date:'2026-03-' + String(20+i).padStart(2,'0') }));
const columns = [
...
export default function Page() {
  return (<div>
    <div className="table-toolbar"><Title level={4} style={{margin:0}}>團體分房表</Title>
    <Button type="primary" icon={<PlusOutlined />} onClick={()=>message.info('新增功能 (待接後端)')}>新增</Button></div>
    {/* === [API] POST /api/group-sales/room === DB: INSERT INTO group_rooms === TODO: [替換] === */}
    {/* === [API] PUT /api/group-sales/room/:id === DB: UPDATE group_rooms SET ... === TODO: [替換] === */}
    {/* === [API] DELETE /api/group-sales/room/:id === DB: DELETE FROM group_rooms WHERE GRUP_CD=:id === TODO: [替換] === */}

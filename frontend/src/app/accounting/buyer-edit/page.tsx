'use client';
/** 買受人修改作業 | L_etord_buyer.asp | DB: e_transfer_orders */
import React from 'react';
import { Typography, Table, Button, Space, Tag, message } from 'antd';
import { PlusOutlined, EditOutlined, DeleteOutlined } from '@ant-design/icons';
const { Title } = Typography;

// === [API] GET /api/accounting/buyer-edit === DB: e_transfer_orders === TODO: [替換] 改為實際 API ===
const mockData = Array.from({length:10},(_,i)=>({ key:String(i+1), id:'TOR' + String(i+1).padStart(4,'0'), name:'買受人修改作業項目' + (i+1), status:i%3===0?'停用':'啟用', date:'2026-03-' + String(20+i).padStart(2,'0') }));
const columns = [
...
export default function Page() {
  return (<div>
    <div className="table-toolbar"><Title level={4} style={{margin:0}}>買受人修改作業</Title>
    <Button type="primary" icon={<PlusOutlined />} onClick={()=>message.info('新增功能 (待接後端)')}>新增</Button></div>
    {/* === [API] POST /api/accounting/buyer-edit === DB: INSERT INTO e_transfer_orders === TODO: [替換] === */}
    {/* === [API] PUT /api/accounting/buyer-edit/:id === DB: UPDATE e_transfer_orders SET ... === TODO: [替換] === */}
    {/* === [API] DELETE /api/accounting/buyer-edit/:id === DB: DELETE FROM e_transfer_orders WHERE TORD_NO=:id === TODO: [替換] === */}

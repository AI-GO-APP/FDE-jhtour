'use client';
/** 電子代轉批次列印 | L_etord_p_AL.asp | DB: reports */
import React from 'react';
import { Typography, Table, Button, Space, Tag, message } from 'antd';
import { PlusOutlined, EditOutlined, DeleteOutlined } from '@ant-design/icons';
const { Title } = Typography;

// === [API] GET /api/accounting/e-print === DB: reports === TODO: [替換] 改為實際 API ===
const mockData = Array.from({length:10},(_,i)=>({ key:String(i+1), id:'ID' + String(i+1).padStart(4,'0'), name:'電子代轉批次列印項目' + (i+1), status:i%3===0?'停用':'啟用', date:'2026-03-' + String(20+i).padStart(2,'0') }));
const columns = [
...
export default function Page() {
  return (<div>
    <div className="table-toolbar"><Title level={4} style={{margin:0}}>電子代轉批次列印</Title>
    <Button type="primary" icon={<PlusOutlined />} onClick={()=>message.info('新增功能 (待接後端)')}>新增</Button></div>
    {/* === [API] POST /api/accounting/e-print === DB: INSERT INTO reports === TODO: [替換] === */}
    {/* === [API] PUT /api/accounting/e-print/:id === DB: UPDATE reports SET ... === TODO: [替換] === */}
    {/* === [API] DELETE /api/accounting/e-print/:id === DB: DELETE FROM reports WHERE =:id === TODO: [替換] === */}

'use client';
/** 應收憑單 | L_aord.asp | DB: receivable_vouchers */
import React from 'react';
import { Typography, Table, Button, Space, Tag, message } from 'antd';
import { PlusOutlined, EditOutlined, DeleteOutlined } from '@ant-design/icons';
const { Title } = Typography;

// === [API] GET /api/accounting/receivable === DB: receivable_vouchers === TODO: [替換] 改為實際 API ===
const mockData = Array.from({length:10},(_,i)=>({ key:String(i+1), id:'AOR' + String(i+1).padStart(4,'0'), name:'應收憑單項目' + (i+1), status:i%3===0?'停用':'啟用', date:'2026-03-' + String(20+i).padStart(2,'0') }));
const columns = [
...
export default function Page() {
  return (<div>
    <div className="table-toolbar"><Title level={4} style={{margin:0}}>應收憑單</Title>
    <Button type="primary" icon={<PlusOutlined />} onClick={()=>message.info('新增功能 (待接後端)')}>新增</Button></div>
    {/* === [API] POST /api/accounting/receivable === DB: INSERT INTO receivable_vouchers === TODO: [替換] === */}
    {/* === [API] PUT /api/accounting/receivable/:id === DB: UPDATE receivable_vouchers SET ... === TODO: [替換] === */}
    {/* === [API] DELETE /api/accounting/receivable/:id === DB: DELETE FROM receivable_vouchers WHERE AORD_NO=:id === TODO: [替換] === */}

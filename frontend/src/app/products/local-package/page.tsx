'use client';
/** 當地套件及自費 | L_package.asp | DB: local_packages */
import React from 'react';
import { Typography, Table, Button, Space, Tag, message } from 'antd';
import { PlusOutlined, EditOutlined, DeleteOutlined } from '@ant-design/icons';
const { Title } = Typography;

// === [API] GET /api/products/local-package === DB: local_packages === TODO: [替換] 改為實際 API ===
const mockData = Array.from({length:10},(_,i)=>({ key:String(i+1), id:'PKG' + String(i+1).padStart(4,'0'), name:'當地套件及自費項目' + (i+1), status:i%3===0?'停用':'啟用', date:'2026-03-' + String(20+i).padStart(2,'0') }));
const columns = [
...
export default function Page() {
  return (<div>
    <div className="table-toolbar"><Title level={4} style={{margin:0}}>當地套件及自費</Title>
    <Button type="primary" icon={<PlusOutlined />} onClick={()=>message.info('新增功能 (待接後端)')}>新增</Button></div>
    {/* === [API] POST /api/products/local-package === DB: INSERT INTO local_packages === TODO: [替換] === */}
    {/* === [API] PUT /api/products/local-package/:id === DB: UPDATE local_packages SET ... === TODO: [替換] === */}
    {/* === [API] DELETE /api/products/local-package/:id === DB: DELETE FROM local_packages WHERE PKG_CD=:id === TODO: [替換] === */}

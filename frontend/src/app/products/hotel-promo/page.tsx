'use client';
/** 旅館預約優惠 | L_hpriv.asp | DB: hotel_promotions */
import React from 'react';
import { Typography, Table, Button, Space, Tag, message } from 'antd';
import { PlusOutlined, EditOutlined, DeleteOutlined } from '@ant-design/icons';
const { Title } = Typography;

// === [API] GET /api/products/hotel-promo === DB: hotel_promotions === TODO: [替換] 改為實際 API ===
const mockData = Array.from({length:10},(_,i)=>({ key:String(i+1), id:'HOT' + String(i+1).padStart(4,'0'), name:'旅館預約優惠項目' + (i+1), status:i%3===0?'停用':'啟用', date:'2026-03-' + String(20+i).padStart(2,'0') }));
const columns = [
...
export default function Page() {
  return (<div>
    <div className="table-toolbar"><Title level={4} style={{margin:0}}>旅館預約優惠</Title>
    <Button type="primary" icon={<PlusOutlined />} onClick={()=>message.info('新增功能 (待接後端)')}>新增</Button></div>
    {/* === [API] POST /api/products/hotel-promo === DB: INSERT INTO hotel_promotions === TODO: [替換] === */}
    {/* === [API] PUT /api/products/hotel-promo/:id === DB: UPDATE hotel_promotions SET ... === TODO: [替換] === */}
    {/* === [API] DELETE /api/products/hotel-promo/:id === DB: DELETE FROM hotel_promotions WHERE HOTEL_CD=:id === TODO: [替換] === */}

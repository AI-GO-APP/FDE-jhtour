'use client';
/** 其他旅遊商品 | L_msln.asp | DB: misc_products */
import React from 'react';
import { Typography, Table, Button, Space, Tag, message } from 'antd';
import { PlusOutlined, EditOutlined, DeleteOutlined } from '@ant-design/icons';
const { Title } = Typography;

// === [API] GET /api/products/misc-product === DB: misc_products === TODO: [替換] 改為實際 API ===
const mockData = Array.from({length:10},(_,i)=>({ key:String(i+1), id:'MSL' + String(i+1).padStart(4,'0'), name:'其他旅遊商品項目' + (i+1), status:i%3===0?'停用':'啟用', date:'2026-03-' + String(20+i).padStart(2,'0') }));
const columns = [
...
export default function Page() {
  return (<div>
    <div className="table-toolbar"><Title level={4} style={{margin:0}}>其他旅遊商品</Title>
    <Button type="primary" icon={<PlusOutlined />} onClick={()=>message.info('新增功能 (待接後端)')}>新增</Button></div>
    {/* === [API] POST /api/products/misc-product === DB: INSERT INTO misc_products === TODO: [替換] === */}
    {/* === [API] PUT /api/products/misc-product/:id === DB: UPDATE misc_products SET ... === TODO: [替換] === */}
    {/* === [API] DELETE /api/products/misc-product/:id === DB: DELETE FROM misc_products WHERE MSLN_CD=:id === TODO: [替換] === */}

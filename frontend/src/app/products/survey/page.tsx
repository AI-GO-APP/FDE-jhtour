'use client';
/** 意見回饋題庫 | L_grup_surv.asp | DB: survey_questions */
import React from 'react';
import { Typography, Table, Button, Space, Tag, message } from 'antd';
import { PlusOutlined, EditOutlined, DeleteOutlined } from '@ant-design/icons';
const { Title } = Typography;

// === [API] GET /api/products/survey === DB: survey_questions === TODO: [替換] 改為實際 API ===
const mockData = Array.from({length:10},(_,i)=>({ key:String(i+1), id:'SUR' + String(i+1).padStart(4,'0'), name:'意見回饋題庫項目' + (i+1), status:i%3===0?'停用':'啟用', date:'2026-03-' + String(20+i).padStart(2,'0') }));
const columns = [
...
export default function Page() {
  return (<div>
    <div className="table-toolbar"><Title level={4} style={{margin:0}}>意見回饋題庫</Title>
    <Button type="primary" icon={<PlusOutlined />} onClick={()=>message.info('新增功能 (待接後端)')}>新增</Button></div>
    {/* === [API] POST /api/products/survey === DB: INSERT INTO survey_questions === TODO: [替換] === */}
    {/* === [API] PUT /api/products/survey/:id === DB: UPDATE survey_questions SET ... === TODO: [替換] === */}
    {/* === [API] DELETE /api/products/survey/:id === DB: DELETE FROM survey_questions WHERE SURV_ID=:id === TODO: [替換] === */}

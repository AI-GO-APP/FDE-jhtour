'use client';
/** 支付店家列表 | L_pay_store.asp | DB: pay_stores */
import React from 'react';
import { Typography, Table, Button, Space, Tag, message } from 'antd';
import { PlusOutlined, EditOutlined, DeleteOutlined } from '@ant-design/icons';
const { Title } = Typography;

// === [API] GET /api/orders/pay-store === DB: pay_stores === TODO: [替換] 改為實際 API ===
const mockData = Array.from({length:10},(_,i)=>({ key:String(i+1), id:'STO' + String(i+1).padStart(4,'0'), name:'支付店家列表項目' + (i+1), status:i%3===0?'停用':'啟用', date:'2026-03-' + String(20+i).padStart(2,'0') }));
const columns = [
...
export default function Page() {
  return (<div>
    <div className="table-toolbar"><Title level={4} style={{margin:0}}>支付店家列表</Title>
    <Button type="primary" icon={<PlusOutlined />} onClick={()=>message.info('新增功能 (待接後端)')}>新增</Button></div>
    {/* === [API] POST /api/orders/pay-store === DB: INSERT INTO pay_stores === TODO: [替換] === */}
    {/* === [API] PUT /api/orders/pay-store/:id === DB: UPDATE pay_stores SET ... === TODO: [替換] === */}
    {/* === [API] DELETE /api/orders/pay-store/:id === DB: DELETE FROM pay_stores WHERE STORE_CD=:id === TODO: [替換] === */}

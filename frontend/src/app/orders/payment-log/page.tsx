'use client';
/** 支付通知紀錄 | L_online_payment_log.asp | DB: payment_logs */
import React from 'react';
import { Typography, Table, Button, Space, Tag, message } from 'antd';
import { PlusOutlined, EditOutlined, DeleteOutlined } from '@ant-design/icons';
const { Title } = Typography;

// === [API] GET /api/orders/payment-log === DB: payment_logs === TODO: [替換] 改為實際 API ===
const mockData = Array.from({length:10},(_,i)=>({ key:String(i+1), id:'PAY' + String(i+1).padStart(4,'0'), name:'支付通知紀錄項目' + (i+1), status:i%3===0?'停用':'啟用', date:'2026-03-' + String(20+i).padStart(2,'0') }));
const columns = [
...
export default function Page() {
  return (<div>
    <div className="table-toolbar"><Title level={4} style={{margin:0}}>支付通知紀錄</Title>
    <Button type="primary" icon={<PlusOutlined />} onClick={()=>message.info('新增功能 (待接後端)')}>新增</Button></div>
    {/* === [API] POST /api/orders/payment-log === DB: INSERT INTO payment_logs === TODO: [替換] === */}
    {/* === [API] PUT /api/orders/payment-log/:id === DB: UPDATE payment_logs SET ... === TODO: [替換] === */}
    {/* === [API] DELETE /api/orders/payment-log/:id === DB: DELETE FROM payment_logs WHERE PAY_LOG_ID=:id === TODO: [替換] === */}

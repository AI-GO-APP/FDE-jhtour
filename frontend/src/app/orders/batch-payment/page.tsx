'use client';
/** 多單支付通知 | L_online_payment.asp | DB: batch_payments */
import React from 'react';
import { Typography, Table, Button, Space, Tag, message } from 'antd';
import { PlusOutlined, EditOutlined, DeleteOutlined } from '@ant-design/icons';
const { Title } = Typography;

// === [API] GET /api/orders/batch-payment === DB: batch_payments === TODO: [替換] 改為實際 API ===
const mockData = Array.from({length:10},(_,i)=>({ key:String(i+1), id:'BAT' + String(i+1).padStart(4,'0'), name:'多單支付通知項目' + (i+1), status:i%3===0?'停用':'啟用', date:'2026-03-' + String(20+i).padStart(2,'0') }));
const columns = [
...
export default function Page() {
  return (<div>
    <div className="table-toolbar"><Title level={4} style={{margin:0}}>多單支付通知</Title>
    <Button type="primary" icon={<PlusOutlined />} onClick={()=>message.info('新增功能 (待接後端)')}>新增</Button></div>
    {/* === [API] POST /api/orders/batch-payment === DB: INSERT INTO batch_payments === TODO: [替換] === */}
    {/* === [API] PUT /api/orders/batch-payment/:id === DB: UPDATE batch_payments SET ... === TODO: [替換] === */}
    {/* === [API] DELETE /api/orders/batch-payment/:id === DB: DELETE FROM batch_payments WHERE BATCH_ID=:id === TODO: [替換] === */}
